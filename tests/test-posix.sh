...
printf '%s\n' '[user]' '    email = existing@example.com' '[include]' '    path = ~/.config/git/.gitconfig' > "$HOME/.gitconfig"
chezmoi init --apply --source "$root"
printf '%s\n' '[tools]' 'node = "22"' >> "$HOME/.config/mise/config.toml"
...
[[ -e $HOME/.config/mise/conf.d/20-dev-cli.toml ]]
[[ -e $HOME/.config/mise/conf.d/00-settings.toml ]]
[[ -e $HOME/.config/mise/config.toml ]]
grep -q '^export MISE_LOCKFILE=false$' "$HOME/.config/zsh/main.zsh"
grep -q '^"aqua:rossmacarthur/sheldon" = "latest"$' "$HOME/.config/mise/conf.d/10-common.toml"
...
[[ ! -e $HOME/.config/mise/conf.d/25-host-ai-cli.toml ]]
...
if [[ $DOTFILES_PROFILE == container ]]; then
  [[ ! -e $HOME/.config/mise/conf.d/30-host-languages.toml ]]
else
  [[ -e $HOME/.config/mise/conf.d/30-host-languages.toml ]]
fi
